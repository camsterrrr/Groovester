...
def deleteSongsOnFileSystemNotInQueue(handler: GroovesterEventHandler):
    """
    ...
    while True:
        ...
        for listItem in listDir:
            absPathToListItem = cwd + listItem

            # Check that the file exists.
            if os.path.exists(absPathToListItem):
                # Check if the file is in use to prevent exceptions being thrown.
                if checkIfFileIsInUse(absPathToListItem):
                    try:
                        os.remove(absPathToListItem)
                        log.debug(
                            "%s %s", DebugMessages._logRemovedFileFromFileSystem, absPathToListItem
                        )
...
async def playDownloadedSongViaDiscordAudio(handler: GroovesterEventHandler):
    ...
    while True:
        ...
        sleep(5) #! Todo: Look into alternatives for this sleep function call. Allows child thread time to open file descriptor. Maybe signal writerCv from speakInVoiceChannel thread?

        #! Todo: Move this to a worker thread that clears the file system of songs not in the queue.
